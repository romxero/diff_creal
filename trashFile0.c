/* 
 * 
 *	This is the new main file right here  
 * 
 * 
 * 
 * 
...
#define USAGE "Cerebro v1.00" //this is the usage of the program
...
#define WRITE_PIPE_PARENT 0 //this is for the parent process in writing

#define WRITE_PIPE_CHILD 0 //this is for the child process in writing
					
#define READ_PIPE_PARENT 1 //this is for the parent process in reading

#define READ_PIPE_CHILD 1 //this is for the child process in reading
...
/* These are the variable that are used in all the functions to process creal json code */ 
	uint numOfCrealNodes  = (uint) json_array_size(*root);
	const char *key, *key0;
	key = key0  = NULL; //setting this all to null 
	...
	*actionItems, *tempForActionAndConditions, *tempForActionAndConditions2, *tempForActionAndConditionsObjects; //json properties, actions, etc
	...
	//~ int i = 0;
	//~ int n = 0;
	int i , n , k, p, o, m, c, t; //these are extras needed
	i = n = k = p = o = m = c = t = 0; //setting the counter variables
	
	
	int insideActionBlockIncrementer,propertyCounter, actionCounter, actionItemCounter, conditionCounter;
	insideActionBlockIncrementer = propertyCounter = actionCounter = actionItemCounter = conditionCounter= 0;
...
int main(int argc, char **argv)
{
					...
					int myReadPipeFd[2]; //file descriptors for pipes
					int myWritePipeFd[2]; //file descriptors for pipes
					

					int writePipe = pipe(myReadPipeFd); //get the pipe data
					int readPipe = pipe(myWritePipeFd); //get the pipe data
					...
					printOutMaxPreprocessedValues(128, 128, 128, 128, &stdout);
					...
					printOutStaticCrealNodes(1024,1024,&stdout); //this prints out the static creal node data
					
					puts(CREAL_CSV_STRUCTS);

					printOutNodeActionAndPropArrays(1024,1024,1024,&stdout); //print out the structures
...
int returnNumberOfCrealProperties(json_t **root)
{
	...
	//~ json_t *jsonPropertiesList, *temp0; //json properties, actions, etc
		...
		//~ int 
		propertiesCount = 0; //this is the property count	
...
int abstractJsonData(json_t **root, FILE **fp)
{
	...
	*actionItems, *tempForActionAndConditions, *tempForActionAndConditions2, *tempForActionAndConditionsObjects; //json properties, actions, etc
	...
	//~ int i = 0;
	//~ int n = 0;
	static int i , n , k, p, o, m, c, t; //these are extras needed
	i = n = k = p = o = m = c = t = 0; //setting the counter variables
	
	
	static int insideActionBlockIncrementer,propertyCounter, actionCounter, actionItemCounter, conditionCounter;
	insideActionBlockIncrementer = propertyCounter = actionCounter = actionItemCounter = conditionCounter= 0;
	...
	//~ /* setting up some temporary variables unsigned integers*/
	//made these variables static
	static creal_t tempPropVar0, tempPropVar1, tempPropVar2, targetValue, conditionsValue;
	tempPropVar0 = tempPropVar1 = tempPropVar2 = targetValue = conditionsValue = 0;
	
	static uint tempVar, idVar, propVar, actVar, condVar, numOfPropertiesInNode, numOfActionsInNode, numOfConditionsInNode, numOfActionItems, actionType, targetID, field, relation;
